...
double getDeterminant(vector<vector<double>> matrix) {
	...
	if (matrix.size() == 1) {
		...
		for (int i = 0; i < matrix.size(); i++) {
			...
			vector<vector<double>> v = matrix;
			v.erase(v.begin());

			for (int j = 0; j < v.size(); j++) {v[j].erase(v[j].begin() + i);}

			first == true ? sum += matrix[0][i] * getDeterminant(v) : sum -= matrix[0][i] * getDeterminant(v);
...
* Matrix.
*/
SquareMatrix::SquareMatrix(const Matrix& right) {
	v = right.v;
}
...
*/
double SquareMatrix::cofactor(int row, int col) {
...
* matrix with NULL as it's only index.
*/
SquareMatrix SquareMatrix::minor(int row, int col) {
...
SquareMatrix SquareMatrix::inverse() {
	...
	if (determinant() == 0) {
		cout << "The given matrix is linearly dependent! No inverse matrix!" << endl;
